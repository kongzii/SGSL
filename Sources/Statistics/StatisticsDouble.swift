...
extension Array where Element == Double {
    public func mean(
        ...
        return gsl_stats_mean(self, stride, count)
    }
    ...
    public func sampleVariance() -> Double {
        return gsl_stats_variance(self, 1, count)
    }

    public func sampleVariance(
        withMean mean: Double
    ) -> Double {
        return gsl_stats_variance_m(self, 1, count, mean)
    }

    public func sampleStandardDeviation(
        withStride stride: Int = 1
    ) -> Double {
        return gsl_stats_sd(self, stride, count)
    }
    ...
    public func sampleStandardDeviation(
        ...
        return gsl_stats_sd_m(self, stride, count, mean)
    }
}
